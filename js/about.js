...
.then(data => {
    if(data.borders) {
        a = data.borders[0].common
        if(data.borders[1]) {
            b = data.borders[1].common
            if(data.borders[2]) {
                c = data.borders[2].common
            }
            else {
                c= "none"
            }
  ...
  wrapper.innerHTML = `<div class="about_wrapper-div container">
        <div class="img_wrapper">
            ...
            <div class="border_place container">
                ...
                <button class="border-btn">${a}</button>
                <button class="border-btn">${b}</button>
                <button class="border-btn">${c}</button>
                </div>
            </div>`;
});
